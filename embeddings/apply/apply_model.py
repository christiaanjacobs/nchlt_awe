...
#sys.path.append(path.join("..", "train_siamese_rnn"))
import models
from dataset_val import GlobalPhone
import samediff
...
def main(options_dict, model_types):
    ...
    dataset_val = GlobalPhone(options_dict)    
...
#        model_dir_fn = path.join(options_dict['model_dir'], 'final_model.pt')
    ...
    if model_type =='siamese_rnn':
        ...
        model_dir_fn = path.join(options_dict['model_dir'], 'final_model.pt')
    if model_type == 'encoder_rnn':
        model = models.encoder_rnn(options_dict)
        model_dir_fn = path.join(options_dict['model_dir'], 'final_model.pt')

    model.load_state_dict(torch.load(model_dir_fn)['state_dict'], strict=False)
#    model.load_state_dict(torch.load(model_dir_fn))
    model.to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--val_lang', type=str, help="GlobalPhone language", choices=nchlt_languages) 
