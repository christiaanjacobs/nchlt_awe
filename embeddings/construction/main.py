"""
Train a one hot mini_batch classifier.
"""
...
import samediff
from models import siamese_rnn, encoder_rnn

import sampler
from sampler import pair_sampler

sys.path.append(path.join(".."))
from link_mfcc import nchlt_languages
...
default_options_dict = {
        ...
        'save_best': True,
        'pair_list_fn': None,
        'remove_overlap': 'xho+zul+ssw+nbl+nso+tsn+sot+afr+eng+tso'
...
def main(options_dict):
    ...
    t_sampler = pair_sampler(dataset_train, options_dict)
    print(datetime.datetime.now())
    ...
    if options_dict["load_model"]:
        ...
        # Initialize model
        model = encoder_rnn(options_dict)
        model.to(device)
    ...
    for epoch in range(options_dict['n_epochs']):
        ...
        if options_dict["shuffle"]:
            t_sampler.minibatch_reconstruction()
        # print(datetime.datetime.now())
...
def check_argv():
    ...
    parser.add_argument(
        "train_lang", type=str,
        help="NCHLT training language {xho, zul, ssw, nbl, nso, tsn, sot, afr, eng, tso, ven}",
        )
    parser.add_argument(
        "--val_lang", type=str, help="validation language",
        choices=nchlt_languages, default=None
        )
    parser.add_argument(
    ...
    parser.add_argument(
        "--pair_list_fn", type=str, help="existing pair list (default: %(default)s)",
        default=default_options_dict["pair_list_fn"]
        )
    parser.add_argument(
        "--shuffle", type=bool, help="shuffle minibatches between epochs(default: %(default)s)",
        default=default_options_dict["shuffle"]
        )
    parser.add_argument(
        "--n_minibatches", type=int, help="number of disjoint batches (default: %(default)s)",
...
if __name__ == '__main__':
    ...
    options_dict["pair_list_fn"] = args.pair_list_fn
    options_dict["shuffle"] = args.shuffle
    options_dict["n_minibatches"] = args.n_minibatches
    print(options_dict["n_minibatches"])
    # Set parameters from loaded pair list
    if options_dict["pair_list_fn"] is not None:
        param_values = options_dict["pair_list_fn"].split("/")[-1].split(".")[:-1] 
        params = ["n_max_pairs", "n_min_tokens_per_type", "n_max_types", "n_max_tokens"]
        for i, value in enumerate(param_values):
            if value.isdigit():
                options_dict[params[i]] = int(value)

        # print(options_dict)

    
    # Model directory
    model_dir = path.join('models', options_dict['train_lang'] + '.' + options_dict['train_tag'])
